...
for ii=1:data_length
     ...
     segment    = filtered_rx_signal(idx_range); % segment of 4 symbols (due to oversampling)
     ...
     % linear
     y = filtered_rx_signal(idx_start+sample_diff : idx_start+sample_diff+1); % containes the two closest samples f(0) and f(1) of the value we try to interpolate
     y_hat = (1-int_diff)*y(1) + int_diff*y(2);
     data(ii) = y_hat;
     
     % cubic
     y2 = filtered_rx_signal(idx_start+sample_diff-1:idx_start+sample_diff+2);
     c = [-1 3 -3 1 ; 3 -6 3 0 ; -2 -3 6 -1 ; 0 6 0 0] * y2;
